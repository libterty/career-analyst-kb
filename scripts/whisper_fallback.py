...
Usage:
    ...
    python scripts/whisper_fallback.py [--model large-v3] [--limit N]

Models (speed vs accuracy tradeoff):
    tiny    — fastest, lowest accuracy
    base    — good for clear speech
    medium  — balanced
    large-v3 — best accuracy, slowest (~1x realtime on CPU, ~10x on GPU)
"""
...
NO_SUB_FILE = ROOT / "data" / "subtitles" / "no_subtitles.txt"
TRANSCRIPT_DIR = ROOT / "data" / "processed" / "transcripts"
AUDIO_DIR = ROOT / "data" / "subtitles" / "audio"
...
def transcribe(audio_path: Path, model: str, language: str) -> str | None:
    """Run Whisper on audio file. Returns transcript text or None on failure."""
    try:
...
def main() -> None:
    ...
    if not NO_SUB_FILE.exists():
        print(f"No no_subtitles.txt found at {NO_SUB_FILE}")
        print("Run: python scripts/audit_subtitles.py first")
        sys.exit(1)

    video_ids = [l.strip() for l in NO_SUB_FILE.read_text().splitlines() if l.strip()]
    if args.limit:
