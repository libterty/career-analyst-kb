...
DEFAULT_TOP_K = 10
OUTPUT_FIELDS = ["chunk_id", "doc_hash", "source", "section", "content", "token_count", "page_number", "video_title", "upload_date", "url"]
...
class MilvusRetriever(IVectorRetriever):
    ...
    def search(self, query_embedding: list[float], top_k: int = DEFAULT_TOP_K, topic: str | None = None) -> list[SearchResult]:
        """執行向量相似度搜索。
        ...
        if not self._ready:
            ...
            if utility.has_collection(COLLECTION_NAME):
                ...
                return []
        try:
            expr = f'section == "{topic}"' if topic else None
