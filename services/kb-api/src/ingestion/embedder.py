...
def _build_schema(embedding_dim: int) -> CollectionSchema:
    ...
    fields = [
        ...
        FieldSchema("upload_date", DataType.VARCHAR, max_length=16),
        FieldSchema("url", DataType.VARCHAR, max_length=128),
    ...
    def embed_and_store(self, chunks: list[Chunk]) -> int:
        ...
        for i in range(0, len(chunks), self.batch_size):
            ...
            data = [
                ...
                [c.content[:4090] for c in batch],
                [c.token_count for c in batch],
                [c.page_hint or 0 for c in batch],
                [c.video_title[:510] for c in batch],
                [c.upload_date[:14] for c in batch],
                [c.url[:126] for c in batch],
                embeddings,
            ]
