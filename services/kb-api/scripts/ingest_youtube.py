...
def _to_domain_chunk(tc: TranscriptChunk, title: str = "", upload_date: str = "") -> Chunk:
    ...
    url = f"https://www.youtube.com/watch?v={tc.video_id}"
    return Chunk(
...
def _ensure_transcripts() -> None:
    ...
    if not txts:
        ...
        subprocess.run(
            [sys.executable, str(SERVICE_ROOT / "scripts" / "vtt_to_text.py")],
            check=True,
        )
