...
class HybridSearchEngine(ISearchEngine):
    ...
    def search(self, query: str, query_embedding: list[float]) -> list[SearchResult]:
        ...
        """
        # 1. 向量搜索（Dense）
        dense_results = self._retriever.search(query_embedding, top_k=self.dense_top_k)
        if not dense_results:
            return []

        # 2. 全語料庫 BM25 搜索（獨立於 Dense，確保關鍵字完全匹配的切塊不被漏掉）
        ...
        if self._bm25_index is not None and self._bm25_corpus:
            ...
            bm25_top_results = [self._bm25_corpus[i] for i in bm25_top_indices]
        else:
        ...
        corpus_size = len(self._bm25_corpus) if self._bm25_corpus else len(dense_results)
